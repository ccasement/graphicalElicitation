...
navbarPage("Graphical Elicitation",
		...
		navlistPanel(
				...
				column(4,
					conditionalPanel(
					  ...
					  p("What is the smallest value you believe your measurement could be
						  (approximately)?"),
					  numericInput("min_pois", label = "Smallest Value", 0L, 
					    width = "50%"),
					  
						br(),
						p("What is the largest value you believe your measurement could be
						  (approximately)?"),
						numericInput("max_pois", label = "Largest Value", 100L, 
							width = "50%"),

					  actionButton("set_pois_inputs", label = "Set Inputs",
					  	class = "btn btn-primary")
					)
				)  # end column
			...
			tabPanel("Start Training", icon = icon("power-off"),
				conditionalPanel(
  				...
  				column(7,
  					h5("Suppose you are presented with a hypothetical dataset of size 50
              for your scenario."),
			  ...
			  conditionalPanel(
  			  ...
  			  column(4,
  			    ...
  			    column(7,
  			      actionButton("generate_pois_training_plots", label = "Generate Graphs", 
  			        class = "btn btn-primary"),
			  ...
			  column(6,
			    ...
			    conditionalPanel(
			      ...
			      numericInput("new_N", label = "Number of Observations", value = 50, 
			        width = "45%"),
			...
			tabPanel("Select Graphs", icon = icon("random"),
				...
				conditionalPanel(
					...
					column(10,
						h4("Select the graph that is most reasonable for your scenario."),
						align = "center",
					...
					column(2,
						...
						tags$head(tags$style(type="text/css", "
							#selections_bern{max-width: 80px;}")),
						align = "center"
					),
				...
				conditionalPanel(
					...
					column(2,
						h4("Selections"),
						verbatimTextOutput("selections_pois"),
						tags$head(tags$style(type="text/css", "#selections{max-width: 80px;}")),
						align = "center"
					),
					
					column(10, plotOutput("select_plots_pois")),			
					
					column(2,
						br(),
						actionButton("plot_1_pois", label = "Graph 1", class = "btn btn-primary", 
							width = "90px"),
						br(),
						br(),
						br(),
						actionButton("plot_2_pois", label = "Graph 2", class = "btn btn-primary", 
							width = "90px"),
						br(),
						br(),
						br(),
						actionButton("plot_3_pois", label = "Graph 3", class = "btn btn-primary", 
							width = "90px"),
						br(),
						br(),
						br(),
						actionButton("plot_4_pois", label = "Graph 4", class = "btn btn-primary", 
							width = "90px"),
						br(),
						br(),
						br(),
						actionButton("plot_5_pois", label = "Graph 5", class = "btn btn-primary", 
							width = "90px"),
						align = "center"
					)
				)
			...
			tabPanel("Compute Prior", icon = icon("flash"),
					...
					column(3,
						checkboxInput("find_prior_prob", label = "Find Prior Probability", 
							value = FALSE),
						
						conditionalPanel(
							condition = "input.find_prior_prob",
							
							column(12,
								h5("Find the probability between two values of the parameter (based on 
									the elicited prior).")
							),
							
							column(6, numericInput("lower_prob", "Lower", 0, width = "120%")),
							
							column(6, numericInput("upper_prob", "Upper", 1, width = "120%")),
			    ...
			    column(3,
			      checkboxInput("change_n", label = "Change Past Sample Size", 
			        value = FALSE),
			      
			      conditionalPanel(
			        condition = "input.change_n",
			        
			        h5("Change the number of times you have seen the scenario of 
			          interest."),
			       
			        numericInput("new_n", label = "Number of Times", value = 20, 
			          width = "70%"),
			        actionButton("previous_n", "Update Sample Size", 
			          class = "btn btn-primary")
			      )
			    ),
			  
					column(3,
						checkboxInput("change_warmup", label = "Change Warm-up", value = FALSE),
						
						conditionalPanel(
							condition = "input.change_warmup",
							
							numericInput("new_warmup", label = "Warm-up", value = 5, 
							  width = "50%"),
							actionButton("warm_up", "Update Warm-up", class = "btn btn-primary")
						)
					),
					
					column(3,
						checkboxInput("plot_options", label = "Change Plot", value = FALSE),
	...
	tabPanel("Help", icon = icon("question-circle"),
		...
		column(4,
			...
			p("1. To start, you must choose the following:"),
			p("(1) a data model,"),
			p("(2) the number of times you have seen the scenario of interest, based 
			  on your previous experiences, and"),
			p("(3) approximate minimum and maximum values for the measurement (for a
        Poisson data model only). This number does not need to be exact; its 
			  purpose is to initialize the algorithm."),
			p("Note: the app defaults the prior family based on the chosen data 
	  ...
	  column(4,
        ...
        on the inputs you specified in the 'Select Model' tab. You 
				must select the graph that is most reasonable for your scenario. Once 
	    ...
	    # compute prior
	    h3("Compute Prior"),
	    p("After making the specified number of selections, you can move to the 'Compute Prior'
	      tab. Here you will see (1) the prior family, (2) the elicited parameters
	      for this family, (3) summaries of the elicited prior, (4) the total 
	      number of selections made, the number kept, and the number discarded 
	      (referred to as the 'warm-up' and defaulted to 15), and (5) a density 
	      plot of the elicited prior."),
	    p("Additional Options:"),
        ...
        of selections kept). The default is 5, which means the final 5 selections
        are kept and used to fit the prior."),
        ...
        model or rates for a Poisson data model). The red circles indicate the 
        values of the parameters that correspond to the selected graphs, and the 
        black circles indicate the values that correspond to the four unselected 
        graphs at each stage."),
			hr(),
